...
export class TrabajadorService {
  private base = `${environment.apiUrl}/pagos/trabajador`;
  ...
  listarPagos(params?: { estado?: string }): Observable<any[]> {
    let httpParams = new HttpParams();
    if (params?.estado) httpParams = httpParams.set('estado', params.estado);
    return this.http.get<any[]>(`${environment.apiUrl}/pagos/trabajador/pagos`, { params: httpParams });
  }
  ...
  listarSuscripciones(params?: { estado?: string; tipo?: 'MEMBRESIA' | 'PLAN' }): Observable<any[]> {
    ...
    return this.http.get<any[]>(`${environment.apiUrl}/pagos/trabajador/suscripciones`, { params: httpParams });
  }

  confirmarSuscripcion(id: number): Observable<any> {
    return this.http.post<any>(`${environment.apiUrl}/pagos/trabajador/suscripciones/${id}/confirmar`, null);
  }

  rechazarSuscripcion(id: number, motivo?: string): Observable<any> {
    let params = new HttpParams();
    if (motivo) params = params.set('motivo', motivo);
    return this.http.post<any>(`${environment.apiUrl}/pagos/trabajador/suscripciones/${id}/rechazar`, null, { params });
  }

  cancelarSuscripcion(id: number, motivo?: string): Observable<any> {
    let params = new HttpParams();
    if (motivo) params = params.set('motivo', motivo);
    return this.http.post<any>(`${environment.apiUrl}/pagos/trabajador/suscripciones/${id}/cancelar`, null, { params });
  }

  eliminarSuscripcion(id: number): Observable<any> {
    return this.http.delete<any>(`${environment.apiUrl}/pagos/trabajador/suscripciones/${id}`);
  }

  eliminarPedido(id: number): Observable<any> {
    return this.http.delete<any>(`${environment.apiUrl}/pagos/trabajador/pedidos/${id}`);
  }
  ...
  iniciarSuscripcion(payload: { membresiaId?: number; planSuscripcionId?: number; monto?: number }): Observable<any> {
    ...
    return this.http.post<any>(`${environment.apiUrl}/suscripciones/iniciar`, payload);
  }
  ...
  crearPreferenciaSuscripcion(suscripcionId: number): Observable<any> {
    ...
    return this.http.post<any>(`${environment.apiUrl}/pagos/mercadopago/preferencia-suscripcion?suscripcionId=${suscripcionId}`, {});
  }

  resumenAsistenciaUsuarios(): Observable<any[]> {
    return this.http.get<any[]>(`${environment.apiUrl}/perfil/asistencia-resumen-usuarios`);
  }

  registrarCheckinPorToken(qrToken: string): Observable<any> {
    const token = encodeURIComponent(qrToken || '');
    return this.http.post<any>(`${environment.apiUrl}/checkin/${token}`, {});
  }
}
